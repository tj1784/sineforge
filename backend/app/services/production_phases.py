...
from backend.app.services.themes import theme_planning_guidance
...
def generate_phase_one(
    ...
    *,
    ...
    if project is not None and project.theme_id != "default":
        resolved_theme = theme_planning_guidance(project)
        creative_direction = dict(package.get("creative_direction") or {})
        creative_direction.update(
            {
                "creative_theme": resolved_theme.snapshot,
                "creative_theme_sha256": resolved_theme.snapshot_sha256,
                "theme_direction": resolved_theme.positive_prompt,
            }
        )
        package["creative_direction"] = creative_direction
        package["source_fidelity_notes"] = [
            *list(package.get("source_fidelity_notes") or []),
            (
                f"Creative theme '{resolved_theme.resolved_theme_id}' is applied to "
                "new planning and generation while the user's source intent remains primary."
            ),
        ]
...
def retain_generated_planning_phase(
    db: Session,
    story_id: UUID,
    phase_number: int,
    *,
    ...
    """Retain truthful generated output for one completed planning phase.

    Project creation previously jumped directly from a baseline to an approval
    for Phases 2-5.  This append-only record proves that the local planning run
    completed and binds the phase snapshot to that run and proposal.
    """

    if phase_number not in {2, 3, 4, 5}:
        raise ProductionPhaseError("Generated planning retention is limited to Phases 2-5.")
    story = _require_story(db, story_id)
    phases = ensure_contract(db, story, commit=False)
    ensure_phase_baselines(db, story, phases=phases, commit=False)
    phase = _require_phase(db, story, phase_number)
    ...
    version = _append_version(
        db,
        phase=phase,
        label=(label or f"Local agent generated phase {phase_number}").strip(),
        notes=(notes or (
            "Generated by the selected local planning agent during orchestration "
            f"run {orchestration_run_id}."
        )).strip(),
        source="generated",
        input_snapshot={
            "reason": "new_project_local_planning",
            "phase_number": phase_number,
    ...
    phase.lifecycle_state = "ready_for_review"
    phase.approved_at = None
    phase.generation_completed_at = now
    phase.is_locked = False
    phase.locked_reason = None
    phase.is_stale = False
    phase.stale_reason = None
    if invalidate_downstream:
        ...
        for item in downstream:
            item.is_stale = True
            item.stale_reason = (
                f"Phase {phase_number} generated a new iteration; review and "
                "regenerate dependent work."
            )
            item.is_locked = True
            item.locked_reason = (
                f"Phase {phase_number} iteration must be approved before dependent "
                "work continues."
            )
    db.add(
        AuditLog(
            entity_type="production_phase_version",
            entity_id=version.id,
            action="local_planning_phase_completed",
            details={
                "story_id": str(story.id),
                "phase_number": phase_number,
                "orchestration_run_id": str(orchestration_run_id),
